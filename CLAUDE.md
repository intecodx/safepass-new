...
│   ├── verify/[id]/                # 인증 확인
│   ├── qr-generate/                # 외국인 QR코드 일괄 생성 (DB등록+승인+QR 원클릭)
│   ├── admin/                      # INTECO 관리자 포털
│   │   ├── login/                  # 로그인
│   │   ├── dashboard/              # 대시보드
...
## 주요 설계 결정 사항 (2026-04 기준)

### 드롭다운 → 네이티브 select
- 출입 신청(`/registration`)의 공사계획 선택: Radix Select → **네이티브 `<select>`** 태그로 변경
- 출입현황(`/admin/entry-status`, `/wie-admin/entry-status`)의 공사명 필터도 동일 적용
- **이유**: Radix Select가 모바일에서 목록이 짤리는 문제 발생. 네이티브 select는 iOS/Android OS 기본 picker 사용으로 짤림 없음
- 옵션 텍스트: `공사명 (시작월/일~종료월/일)` 형식으로 간결화 (회사명 중복 제거)

### 출입 로그 쿼리 최적화
- `getAccessLogs(days?)` — `days` 파라미터로 최근 N일만 쿼리 가능
- 경비실/관리자 API에서 **최근 10일**분만 조회 (`getAccessLogs(10)`)
- **이유**: 기존에는 전체 데이터를 매번 가져와서 Supabase egress 낭비. 경비실에서 보는 건 오늘~최근 며칠이므로 기능 영향 없음
- 과거 데이터는 DB에 그대로 보존 (쿼리 범위만 제한)

### 경비실 대시보드 polling
- INTECO/WIE 경비실 대시보드: **5분(300초)** 간격으로 자동 새로고침
- **이유**: 기존 2초 polling → Supabase 무료 플랜 egress 한도 초과 방지

### 외국인 QR코드 일괄 생성 (`/qr-generate`)
- 한국 핸드폰 없는 외국인을 위해 **DB 등록 → 자동 승인 → QR 생성 → A4 인쇄** 원클릭 처리
- 연락처: `외국인(번호없음)01` ~ `외국인(번호없음)NN` (DB UNIQUE 제약 충족)
- QR 데이터에 실제 DB userId가 포함되어 경비실 스캔 시 정상 출퇴근 기록
- QR 이미지: 외부 API `api.qrserver.com` 사용
