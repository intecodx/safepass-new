...
export default function WieConstructionPlansPage() {
  ...
  const [formData, setFormData] = useState({
    title: "",
    description: "",
    company: "WIE", // Default value set to WIE for WIE admin
    work_company: "",
    start_date: "",
    end_date: "",
    site_manager: "",
  ...
  useEffect(() => {
    console.log("🚀 WIE ConstructionPlansPage 컴포넌트 마운트됨")
    fetchPlans()
  }, [])

  const fetchPlans = async () => {
    try {
      console.log("🔄 WIE 공사계획 목록 조회 시작...")
      setLoading(true)
      ...
      console.log("📋 WIE API에서 받은 공사계획 데이터:", data)
      ...
      if (expiredPlans.length > 0) {
        ...
        const updatePromises = expiredPlans.map(async (plan: ConstructionPlan) => {
          try {
            const updateResponse = await fetch(`/api/wie-admin/construction-plans/${plan.id}`, {
              method: "PUT",
              headers: {
        ...
        const updatedResponse = await fetch("/api/wie-admin/construction-plans")
      ...
      const plansWithUserCount = await Promise.all(
        data.map(async (plan: ConstructionPlan) => {
          try {
            const userResponse = await fetch(`/api/wie-admin/construction-plans/${plan.id}/users`)
            if (userResponse.ok) {
      ...
      console.log(`✅ 총 ${plansWithUserCount.length}개의 WIE 공사계획 로드 완료`)
  ...
  const validateForm = () => {
    ...
    if (!formData.site_manager.trim()) {
      errors.site_manager = "현장관리자를 입력해주세요."
    }
    if (!formData.supervisor.trim()) {
      errors.supervisor = "감독자를 입력해주세요."
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          title: "",
          description: "",
          company: "WIE", // Reset to WIE default value
          work_company: "",
          start_date: "",
          end_date: "",
          site_manager: "",
        ...
        if (result.success && result.id) {
          const newPlan: ConstructionPlan = {
            ...
            work_company: formData.work_company,
          ...
          console.log("✅ 새 WIE 공사계획이 로컬 상태에 추가됨:", newPlan)
        }
  ...
  const handleEditStart = (plan: ConstructionPlan) => {
    ...
    setEditFormData({
      ...
      work_company: plan.work_company || "",
  ...
  const handleEditSave = async () => {
    ...
    try {
      const response = await fetch(`/api/wie-admin/construction-plans/${editingPlan.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async (planId: number, planTitle: string) => {
    ...
    try {
      const response = await fetch(`/api/wie-admin/construction-plans/${planId}`, {
        method: "DELETE",
      })
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-20 lg:relative">
        <div className="max-w-6xl mx-auto px-4 py-3 sm:py-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h1 className="text-lg sm:text-xl font-semibold text-gray-900">공사계획 관리</h1>
          </div>
        </div>
      </header>
            ...
            <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              <CardHeader className="bg-purple-50 border-b">
                <CardTitle className="flex items-center space-x-2 text-lg sm:text-xl">
                  ...
                  <span className="text-purple-900">공사계획 수정</span>
                </CardTitle>
              </CardHeader>
                ...
                <form className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label htmlFor="edit-title" className="text-base font-medium text-gray-700">
                        작업명 *
                      </Label>
                      <Input
                        id="edit-title"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="edit-company" className="text-base font-medium text-gray-700">
                        발주처 *
                      </Label>
                      <Input
                        id="edit-company"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label htmlFor="edit-description" className="text-base font-medium text-gray-700">
                        공사계획 및 내용 *
                      </Label>
                      <Textarea
                        id="edit-description"
                        ...
                        className="min-h-[120px] text-base border-2 focus:border-purple-500"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="edit-work-company" className="text-base font-medium text-gray-700">
                        작업업체 *
                      </Label>
                      <Input
                        id="edit-work-company"
                        type="text"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      <Label htmlFor="edit-start-date" className="text-base font-medium text-gray-700">
                        공사 시작일 *
                      </Label>
                      <Input
                        id="edit-start-date"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="edit-end-date" className="text-base font-medium text-gray-700">
                        공사 종료일 *
                      </Label>
                      <Input
                        id="edit-end-date"
                    ...
                    <div className="space-y-2">
                      <Label className="text-base font-medium text-gray-700">공사 상태</Label>
                      <Select
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label htmlFor="edit-site-manager" className="text-base font-medium text-gray-700">
                        현장관리자 *
                      </Label>
                      <Input
                        id="edit-site-manager"
                        type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onBlur={(e) => {
                        if (e.target.value === "") {
                          e.target.placeholder = "공사 내용, 작업 범위, 주요 작업 사항 등을 상세히 입력해주세요"
                        }
                      }}
                    />
                  </div>
