...
export default function WieAdminStatusPage() {
  ...
  const handleStatusChange = async (userId: number, newStatus: string) => {
    ...
    try {
      ...
      switch (newStatus) {
        case "approved":
          endpoint = `/api/wie-admin/applications/${userId}/approve`
          body = { phone: user.phone }
          break
        case "rejected":
          endpoint = `/api/wie-admin/applications/${userId}/reject`
          const rejectionReason = prompt("반려 사유를 입력하세요 (선택사항):")
          body = { phone: user.phone, reason: rejectionReason || "" }
          break
        case "pending":
          // 대기 상태로 되돌리기 API (새로 만들어야 함)
          endpoint = `/api/wie-admin/applications/${userId}/reset`
          break
        default:
          return
      }
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center">
            <Link href="/wie-admin/dashboard" className="mr-4">
              <ArrowLeft className="w-6 h-6 text-gray-600" />
            </Link>
            <h1 className="text-xl font-semibold text-gray-900">승인 조회/변경</h1>
          </div>
          <Button
                          ...
                          <div className="flex space-x-2">
                            {getAvailableStatusChanges(user.status).map((statusChange) => (
                              <button
                                ...
                                className="px-3 py-1 text-sm border border-purple-300 rounded hover:bg-purple-50 transition-colors text-purple-700"
                              >
                                {statusChange.label}
                              </button>
                            ))}
