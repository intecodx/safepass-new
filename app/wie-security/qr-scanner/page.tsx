...
export default function WieQRScanner() {
  ...
  const processQRCode = async (qrText: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/wie-security/verify-qr", {
        method: "POST",
      ...
      if (response.ok && result.success) {
        ...
        setModalMessage(`❌ 출입 거부\n\n${result.message || result.error || "유효하지 않은 QR코드입니다."}`)
      }
    } catch (error) {
        ...
        <nav className="flex mb-6" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            <li className="inline-flex items-center">
              <a href="/" className="inline-flex items-center text-sm font-medium text-gray-700 hover:text-purple-600">
                홈
              </a>
            </li>
            <li>
              <div className="flex items-center">
                ...
                <a href="/wie-security" className="text-sm font-medium text-gray-700 hover:text-purple-600">
                  WIE 경비실 시스템
                </a>
              </div>
            </li>
          ...
          <div className="space-y-6">
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              ...
              <button
                ...
                className="w-full px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50 transition-all duration-200 shadow-md"
              >
                수동 입력 처리
              </button>
            </div>
          ...
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex justify-center items-center">
            <div className="relative p-8 bg-white w-full max-w-lg mx-auto rounded-xl shadow-2xl border max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-right mt-6">
                <button
                  onClick={closeModal}
                  className="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50 transition-all duration-200 shadow-md"
                >
                  확인
                </button>
              </div>
