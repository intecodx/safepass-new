...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] WIE 수동 재출근처리 요청:", { userId })
    ...
    if (error) {
      console.error("[v0] WIE 재출근 기록 생성 실패:", error)
      return NextResponse.json(
        {
          success: false,
    ...
    console.log("[v0] WIE 수동 재출근처리 완료:", user.name)
    ...
    console.error("[v0] WIE 수동 재출근처리 실패:", error)
    return NextResponse.json(
      {
        success: false,
