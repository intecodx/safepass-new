-- 🚀 Safe Pass 완전 Supabase 마이그레이션 스크립트
...
DROP POLICY IF EXISTS "Enable all access for construction_plans" ON construction_plans;
DROP POLICY IF EXISTS "Enable all access for users" ON users;
DROP POLICY IF EXISTS "Enable all access for admins" ON admins;
DROP POLICY IF EXISTS "Enable all access for access_logs" ON access_logs;
...
CREATE POLICY "Enable insert access for authenticated users" ON admins FOR INSERT WITH CHECK (true);
...
-- 샘플 사용자 데이터
INSERT INTO users (name, phone, email, nationality, passport_number, birth_date, gender, construction_plan_id, roles, vehicle_info, status) VALUES
('김근로자', '010-1234-5678', 'kim@example.com', 'KR', NULL, '1990-01-01', 'male', 1, 
 '{"site_manager": true, "vehicle_owner": false}', NULL, 'pending'),

('John Smith', '010-9876-5432', 'john@example.com', 'US', 'P123456789', '1985-05-15', 'male', 2, 
 '{"site_manager": false, "vehicle_owner": true}', 
 '{"number": "12가3456", "type": "승용차"}', 'approved'),

('이작업자', '010-1111-2222', 'lee@example.com', 'KR', NULL, '1988-03-20', 'male', 1, 
 '{"site_manager": false, "vehicle_owner": false}', NULL, 'approved'),

('박기술자', '010-3333-4444', 'park@example.com', 'KR', NULL, '1992-07-10', 'female', 3, 
 '{"site_manager": true, "vehicle_owner": true}', 
 '{"number": "34나5678", "type": "SUV"}', 'pending'),

('Wang Lei', '010-5555-6666', 'wang@example.com', 'CN', 'C987654321', '1987-11-25', 'male', 4, 
 '{"site_manager": false, "vehicle_owner": false}', NULL, 'approved'),

('최안전관리자', '010-7777-8888', 'choi@example.com', 'KR', NULL, '1980-05-15', 'male', 5, 
 '{"site_manager": true, "vehicle_owner": true}', 
...
(2, '2024-02-03 08:30:00+09', NULL),
(3, '2024-02-03 09:15:00+09', NULL);
...
SELECT 
    '🎉 Safe Pass Supabase 마이그레이션 완료!' as status,
